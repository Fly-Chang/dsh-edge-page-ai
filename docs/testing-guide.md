...
3. 按 `Ctrl+Shift+B` 显示书签栏，把页面里的「DSH 整页翻译」链接拖到书签栏；
   拖不动时手动新建收藏夹，地址填页面文本框中的代码。
4. 打开 `http://127.0.0.1:8000/sample-page.html`，点击该书签。
5. 右下角出现「DSH 页面助手」面板，状态为“就绪：mock-model（协议 v1）”。
6. 点击「整页翻译」。
