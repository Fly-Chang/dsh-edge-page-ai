...
- 翻译进行中会出现「停止」按钮，点击后强制终止剩余批次，已完成的译文保留。
...
7. 保存设置。
8. 打开任意 http/https 页面，**页面加载时不会自动出现面板**（手动模式）。
   点击扩展工具栏图标，或按 `Alt+Shift+D`，面板才出现/隐藏。
   （快捷键若冲突，在 `edge://extensions/shortcuts` 重新分配。）

薄壳扩展使用隔离世界直接加载本地 bundle，不向页面注入 `<script>`，因此可用于
书签模式受限的外部站点（严格 CSP / 本地网络访问限制，见 BUG-005、BUG-013）。
...
- 每 30 个文本片段为一批，3 路并发请求；
- 每完成一批立即回填，页面会“逐块”变中文，不需要等整页完成；
- 翻译请求关闭思考（`disableThinkingForTranslate: true`），对话请求仍保留低思考。
